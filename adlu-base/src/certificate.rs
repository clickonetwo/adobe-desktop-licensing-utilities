...
mod test {
    ...
    fn try_load_and_compare_certificates() {
        ...
        assert_eq!(
            remove_ascii_whitespace(&pfx_key_str),
            remove_ascii_whitespace(&key_str)
        );
        assert_eq!(
            remove_ascii_whitespace(&pfx_cert_str),
            remove_ascii_whitespace(&cert_str)
        );
        let pem_data =
        ...
        assert_eq!(
            remove_ascii_whitespace(&pem_key_str),
            remove_ascii_whitespace(&key_str)
        );
        assert_eq!(
            remove_ascii_whitespace(&pem_cert_str),
            remove_ascii_whitespace(&cert_str)
        );
